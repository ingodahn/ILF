...
# Lenovo:
$files="D:\\\\User\\CloudStation\\ILF\\ilf16\\tmp\\";
$reffile="$files.proofout.$jobNr.ref";
...
close(REFFILE);
# unlink($reffile);
open(PROOFOUT, "<$proofout") or die "$progname: Cannot open $proofout";
# copy STDOUT to another filehandle
open (my $STDOLD, '>&', STDOUT);

# redirect STDOUT to .proofout.jobNr.tex
open (STDOUT, '>', "$files.proofout.$jobNr.tex");
...
while (<PROOFOUT>) {
	chomp;
	if (/^ilf_include (.*)$/) {
	# Translating filenames for Windows
		$includefile=$1;
		$includefile =~ s/^\/\/(.)\//$1:\\\\\\\\/;
		$includefile =~ s/\//\\\\/g;
